...
get_stateful_dev() {
	cgpt find -l CROSMIDI_ROOT | head -n 1
}
...
init_crosmidi() {
	...
	printf "\033[1;96m"
	printf "\n\nCROSMIDI is loading...\n\n"
	echo "Mounting CROSMIDI_ROOT..."
	echo "Creating directory /crosmidi-root..."
	mkdir /crosmidi-root
	if [ -z $(get_stateful_dev) ]; then
		echo "Failed to find CROSMIDI_ROOT! Exiting..."
		exit 1
	fi
	echo "CROSMIDI_ROOT is: $(get_stateful_dev)"
	echo "Attempting to mount $(get_stateful_dev) to /crosmidi-root..."
	echo "$(mount $(get_stateful_dev) /crosmidi-root)"
	local src_path="/crosmidi-root"
	local ret=0
	if [ -d "${src_path}" ]; then
